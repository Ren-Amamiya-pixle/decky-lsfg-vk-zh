...
import t, { zhError } from "../i18n/i18n";
          ...
          {status === "error" && (
            <>
              <PanelSectionRow>
                <Field
                  label="启动选项不可用"
                  description={error ? zhError(error) : "Steam 未提供可读取的启动选项。"}
                />
              </PanelSectionRow>
              <PanelSectionRow>
                <ButtonItem layout="below" onClick={() => void refresh()}>重试</ButtonItem>
              </PanelSectionRow>
            </>
          )}
